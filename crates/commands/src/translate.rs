...
//!
//! ```no_run
//! use nettoolskit_commands::translate::TranslateRequest;
//!
//! let request = TranslateRequest {
//!     from: "csharp".to_string(),
//!     to: "typescript".to_string(),
//!     path: "templates/class.cs.hbs".to_string(),
//! };
//! ```
...
/// * `source_strategy` - Source language strategy
/// * `target_strategy` - Target language strategy (.NET)
///
/// # Returns
///
...
/// Converts template content to target language conventions
...
impl ConventionConverter {
    fn to_dotnet(
        ...
        convert_to_dotnet_conventions(content, source_strategy, target_strategy)
    }
}

/// Writes converted content to output file
struct OutputWriter;
...
///
/// Finds patterns like {{variableName}} in the template
...
/// Convert template content to .NET conventions
///
/// Applies .NET-specific naming and structure conventions
fn convert_to_dotnet_conventions(
    content: &str,
    _source_strategy: &Arc<dyn LanguageStrategy>,
    _target_strategy: &Arc<dyn LanguageStrategy>,
) -> String {
    // For .NET, we apply PascalCase conventions and add C# specific patterns
    let mut result = content.to_string();

    // Convert common naming patterns to PascalCase
    ...
    // Add C# XML doc comment pattern if not present
...
/// Calculate output file path based on input and target extension
///
/// Replaces the file extension with the target language extension
fn calculate_output_path(input_path: &str, target_extension: &str) -> String {
    ...
    // Check if clean_name already has the target extension
