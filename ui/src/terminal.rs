...
impl TerminalLayoutInner {
    ...
    fn render_footer_locked(&self, state: &LayoutState) -> io::Result<()> {
        let mut stdout = io::stdout();
        let origin = cursor::position().unwrap_or((0, state.scroll_top));

        reset_scroll_region_full()?;
        execute!(
            stdout,
            cursor::MoveTo(0, state.footer_start),
            Clear(ClearType::FromCursorDown)
        )?;

        let separator = "-".repeat(state.width as usize);
        queue!(stdout, Print(&separator))?;

        for idx in 0..state.log_capacity {
            let content = state
                .logs
                .get(idx)
                .map_or(String::new(), |entry| pad_to_width(entry, state.width));
            queue!(
                stdout,
                cursor::MoveTo(0, state.footer_start + 1 + idx as u16),
                Print(content)
            )?;
        }
        ...
        queue!(
            ...
            cursor::MoveTo(
                0,
                state.footer_start + state.footer_height.saturating_sub(1)
            ),
            Print(&separator)
        )?;

        apply_scroll_region(state.scroll_top, state.scroll_bottom)?;
    ...
    fn append_log_line(&self, line: &str) -> io::Result<()> {
        ...
        if state.logs.len() == state.log_capacity {
            state.logs.pop_front();
        }

        let truncated = truncate_to_width(&cleaned, state.width);
